  ...
  <div class="container mx-auto px-10">
    ...
    @if($categorias)
      @foreach($categorias as $categoria)
        <h3 class="text-2xl text-slate-400 pt-6">{{ $categoria->name }}</h3>
        @if($subcategorias)
          @foreach($subcategorias as $subCategoria)
            @if($subCategoria->category_id == $categoria->id)
              <h3 class="text-5xl text-lime-600 font-bold pb-4">{{ $subCategoria->name }} - {{ $subCategoria->products->count() }}</h3>
